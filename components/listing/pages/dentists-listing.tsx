...
type DentistsListingProps = {
  currentPage: number;
};

export async function generateDentistsListingMetadata(currentPage: number): Promise<Metadata> {
  ...
  const url = absoluteUrl(listingCanonicalPath('/dentists', currentPage));
...
export async function DentistsListing({ currentPage }: DentistsListingProps) {
  const limit = 20;
