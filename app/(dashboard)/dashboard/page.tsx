...
function Stat({ name, value, path, isLoading = false }: StatProps) {
  return (
    <div className="relative overflow-hidden rounded-lg border border-gray-200 bg-white px-4 pb-12 pt-5 shadow dark:border-gray-700 dark:bg-gray-900/60 sm:px-6 sm:pt-6">
      <dt>
        <p className="font-regular truncate text-sm text-gray-500 dark:text-gray-400">{name}</p>
      </dt>
      <dd className="flex items-baseline pb-6 sm:pb-7">
        {isLoading ? (
          <Skeleton className="h-8 w-16" />
        ) : (
          <p className="text-2xl font-semibold text-gray-900 dark:text-gray-100">{value ?? 0}</p>
        )}
        <div className="absolute inset-x-0 bottom-0 bg-gray-50 px-4 py-4 dark:bg-gray-800/50 sm:px-6">
          <div className="text-sm">
            <a href={path} className="font-medium text-blue-600 hover:text-blue-500 dark:text-blue-400 dark:hover:text-blue-300">
